...
extern ofstream cache_performance_file;
extern char * hexfile;

long long icache_latency;
...
std::string Cache::getByte(uint32_t addr, uint32_t *cycles) {
  ...
  if ((blockId = this->getBlockId(addr)) != -1) {
    ...
    if (addr % 4 == 3) 
      {this->statistics.numHit++; icache_latency = 2;}
  ...
  if (addr % 4 == 3) 
    {this->statistics.numMiss++; icache_latency = 8;}
...
void Cache::setByte(uint32_t addr, std::string val, uint32_t *cycles) {
  this->referenceCounter++;
  ...
  if (this->writeAllocate) {
    ...
    if (this->lowerCache == nullptr) {
      this->memory->writeSingleByte(addr, val);
    } else {
...
void Cache::initCache() {
  ...
  for (uint32_t i = 0; i < this->blocks.size(); ++i) {
    ...
    b.lastReference = 0;
    b.data = std::vector<std::string>(b.size);
  }
}
...
void Cache::loadBlockFromLowerLevel(uint32_t addr, uint32_t *cycles) {
  ...
  for (uint32_t i = blockAddrBegin; i < blockAddrBegin + blockSize; ++i) {
    if (this->lowerCache == nullptr) {
      b.data[i - blockAddrBegin] = this->memory->readSingleByte(i);
...
void Cache::writeBlockToLowerLevel(Cache::Block &b) {
  ...
  if (this->lowerCache == nullptr) {
    for (uint32_t i = 0; i < b.size; ++i) {
      this->memory->writeSingleByte(addrBegin + i, b.data[i]);
    }
  } else {
    for (uint32_t i = 0; i < b.size; ++i) {
